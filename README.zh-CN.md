...
CPIdeas Plus Viewer 是用于 product result 的本地只读 Web 查看器。它面向评审、客户和项目相关方，用来浏览已经交付的题目包，而不暴露私有 AI 生成流程。

## 支持什么

- 浏览单个题目 product result，或浏览一批题目 product result。
- 查看题面、输入输出格式、约束、样例和预期解法说明。
- 如果 product result 中包含本地化文件，可以查看多语言 statement/review/preview。
...
批量 product result 会在根目录包含 `batch_index.json`，并且每个题目一个子目录。

完整契约见 `docs/PRODUCT_RESULT.zh-CN.md`。
...
- 只交付 product result，不交付私有生成产物。
- 确认每个 result 包含 `run.json` 和 `package/config/package.json`。
